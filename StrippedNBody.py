...
time_run = 2 * Gyr  # Total time simulation is run for.
if rewind:
...
def return_info():
    ...
    print("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}\t{12}\t{13}\t{14}\t{15}\t{16}\t{17}".format(
          objects[pri].xyz[0], objects[pri].xyz[1], objects[pri].xyz[2], objects[pri].v_xyz[0], objects[pri].v_xyz[1],
          objects[pri].xyz[2], objects[sec].xyz[0], objects[sec].xyz[1], objects[sec].xyz[2], objects[sec].v_xyz[0],
          objects[sec].v_xyz[1], objects[sec].xyz[2], pericentre, time_of_pericentre, separation, vxg1, vyg1, vzg1))
